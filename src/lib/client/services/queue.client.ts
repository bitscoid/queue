...
// Buat queue baru
export async function createQueue(data: z.infer<typeof queueSchema>): Promise<QueueOutput> {
    const payload = queueSchema.parse(data); // validasi input
    return fetcher<QueueOutput>("/api/queues", {
        method: "POST",
        body: JSON.stringify(payload),
        headers: { "Content-Type": "application/json" },
    }, queueResponseSchema); // validasi output
}

// Update queue
export async function updateQueue(id: number, data: z.infer<typeof queueUpdateSchema>): Promise<QueueOutput> {
    const payload = queueUpdateSchema.parse(data); // validasi input
    return fetcher<QueueOutput>(`/api/queues/${id}`, {
        method: "PUT",
        body: JSON.stringify(payload),
        headers: { "Content-Type": "application/json" },
    }, queueResponseSchema); // validasi output
